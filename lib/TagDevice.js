...
const { devicesByMac, normalizeMac } = require('./devices');
...
class TagDevice extends Device {
  ...
  async onDeleted() {
    ...
    if (this.cameraImagePromise) {
      const image = await this.cameraImagePromise.catch(() => null);
      this.cameraImagePromise = null;
      if (image) {
        await image.unregister().catch((error) => {
          this.error('Failed to unregister camera image:', error);
        });
      }
    }

    // Older versions let one tag be paired on two drivers, and both devices
    // then share this file. Removing one must not blank the other's tile.
    if (this.screenshotSharedWithAnotherDevice()) {
      this.log('Another device shows the same tag, keeping its screenshot');
      return;
    }
  ...
  screenshotSharedWithAnotherDevice() {
    try {
      const mac = normalizeMac(this.getData().id);
      const others = (devicesByMac(this.homey).get(mac) || []).filter((device) => device !== this);
   ...
   *
   * The promise is what is kept, not the Image, so two updates that both
   * arrive before the first registration finished share it instead of each
   * registering an Image and leaking one of them.
   */
  getCameraImage() {
    if (!this.cameraImagePromise) {
      this.cameraImagePromise = this.registerCameraImage().catch((error) => {
        // Let the next update try again rather than fail forever.
        this.cameraImagePromise = null;
        throw error;
      });
    }
    return this.cameraImagePromise;
  }
  ...
  async registerCameraImage() {
    ...
    try {
      await this.setCameraImage(id, id, image);
    } catch (error) {
      await image.unregister().catch(() => {});
      throw error;
    }
    return image;
  }
