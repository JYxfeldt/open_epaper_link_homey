...
function unwrap(data, tagType) {
  ...
  if (data.length > 4) {
    ...
    if (declared > 6 && declared <= 8 * 1024 * 1024) {
      try {
        // Capped at the declared length: without the cap a few kB of crafted
        // input could inflate to far more memory than the app has.
        const inflated = zlib.inflateSync(data.subarray(4), { maxOutputLength: declared });
