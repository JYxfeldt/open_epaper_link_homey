...
// LOCAL ONLY: Business Source Licensed, must never be published. See the note
// in decodeRawImage and the commit that added it.
...
function decodeRawImage(data, tagType) {
  ...
  // LOCAL ONLY - see lib/vendor/g5decoder.js. Upstream deliberately refuses
  // g5-compressed buffers because the only decoder available is under the
  // Business Source License, which is source-available rather than open source
  // and cannot be published to the Homey App Store. This branch is not
  // published, so it uses it: six of the tags on this AP are g5 and would
  // otherwise have no preview at all.
  if (body.length < planeBytes) {
    if (!(tagType && tagType.g5_compression)) {
      throw new Error(`Cannot decode raw buffer: body is ${body.length} bytes,`
        + ` need ${planeBytes} for ${width}x${height}`);
    }
    // For bpp 2 the planes are one stream of double height, not two streams.
    const streamHeight = height * Math.max(1, bpp);
    const decoded = processG5(body, width, streamHeight);
    if (!decoded) throw new Error('Cannot decode raw buffer: G5 decoding failed');
  ...
  return {
    width, height, rgb, container: usedContainer, bpp, planes, rotateDegrees,
  };
}
